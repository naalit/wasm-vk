...
impl Visitor for Interpreter {
    ...
    fn visit(&mut self, op: AOp<Value>) -> Value {
        ...
        match op {
            ...
            Store(ptr, val) => if let Value::I32(ptr) = *ptr {
                self.memory.write().unwrap()[ptr as usize / 4] = *val;
                Value::I32(0)
            } else {
                panic!("{:?} is not a pointer", ptr);
            }
            Load(ptr) => if let Value::I32(ptr) = *ptr {
...
pub fn interpret(buffer: &[u32], module: &wasm::Module) -> Vec<u32> {
    ...
    for i in 0..buffer.len() {
        ...
        am.visit(0, vec![TVal { val: Value::I32(i as u32), ty: WasmTy::I32 }], &mut Interpreter {
                    memory: mem.clone()
                }).unwrap();
    }
    let m = mem.read().unwrap();
    m.iter().map(|x| if let Value::I32(x) = x { *x } else { panic!("Got {:?} in buffer", x) }).collect()
}
